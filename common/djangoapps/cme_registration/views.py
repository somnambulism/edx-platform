...
def cme_create_account(request, post_override=None):
    ...
    for var in ['username', 'email', 'password', 'name']:
        if var not in post_vars:
            js['value'] = "Error (401 {field}). E-mail us.".format(field=var)
            js['field'] = var
    ...
    try:
        ...
        js['value'] = "Valid e-mail is required."
        js['field'] = 'email'
        return HttpResponse(json.dumps(js))
    ...
    try:
        ...
        js['value'] = "Username should only consist of A-Z and 0-9, with no spaces."
        js['field'] = 'username'
        return HttpResponse(json.dumps(js))
    ...
    email_dict = {'name': post_vars['name'],
